...
Seed 6304 throughout. Environment in `task2/requirements.txt`. Seed, domains, split and image sizes are in `task2/configs/task2.yaml`; optimizer and schedule (AdamW, learning rate 1e-4, weight decay 1e-4, at most 30 epochs, patience 5 on mean source-validation macro-F1) are set in cell 6. Every update uses 8 examples from each source domain plus 24 target images and BatchNorm running statistics stay frozen at their ImageNet values.
Outputs are in `task2/results/`: `task2_results.json` (all reported numbers: per-domain and target metrics, domain separability, per-class target accuracy, the λ_MMD study, hyperparameters and training histories) and `figures/`. The run also writes `checkpoints/source_only.pt`, reused unchanged as the Task 3 ERM baseline, and `shared/splits/pacs_sketch_seed6304.json`, the split protocol shared by Tasks 2 and 3. The split file is committed; the checkpoint is not, and is regenerated by re-running.
...
all reused from Task 2. Settings are in `task3/configs/task3.yaml`. Environment in `task3/requirements.txt`. Checkpoints are chosen on the
